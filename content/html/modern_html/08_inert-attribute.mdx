...
<Callout type="warning">
  **자주 틀리는 점:** `inert`를 `display: none`이나 `visibility: hidden`과 같은 것으로 오해하면 안 됩니다. 저 둘은 요소를 화면에서 아예 지우거나(공간까지 차지하지 않거나, 공간은 차지하되 안 보이게) 하는 CSS 속성이고, `inert`는 시각적으로는 그대로 두고 오직 **상호작용 경로만** 차단하는 HTML 속성입니다. 셋의 차이는 CSS 과목의 `visibility` vs `display` vs `hidden` 비교(`/css/css_fundamentals` 참조)와 함께 놓고 봐야 헷갈리지 않습니다.
</Callout>
