...
<Quiz
  questionNumber={2}
  question="다음 코드에서 s1과 s2가 생성된 후의 관계에 대한 설명으로 옳은 것은? Student s1 = new Student('민준', 90); Student s2 = new Student('서연', 85);"
...
<Quiz
  questionNumber={3}
  question="다음 코드의 실행 결과로 옳은 것은? Student s1 = new Student('민준', 90); Student s3 = s1; s3.score = 100; System.out.println(s1.score);"
...
<Quiz
  questionNumber={5}
  question="다음 코드의 실행 결과로 옳은 것은? Box b = new Box(); System.out.println(b.count); System.out.println(b.label); (Box는 int count, String label 필드만 선언하고 초기화하지 않았다)"
